...
import type { UseAppLazyQueryOptions } from './UseAppLazyQuery';
import type { UseAppQueryOptions } from './UseAppQuery';
import type { AppSuspenseQueryOptions } from './UseAppSuspenseQuery';
...
describe('defineEndpoint', () => {
  ...
  it('spreads into every key-consuming option shape (compile-time)', () => {
    ...
    const mapped: UseAppQueryOptions<number, CodeDto[]> = { ...listCodes, map: (raw) => raw.length };
  ...
  it('rejects malformed defs, mismatched raw types, and wrong factory args at compile time', () => {
    ...
    const badMap: UseAppQueryOptions<number, string> = { ...listCodes, map: (raw) => raw.length };
