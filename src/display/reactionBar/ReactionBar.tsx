...
function ReactionChip({ isReactedByMe, className, ...rest }: ChipProps) {
  return (
    <button
      type="button"
      data-active={isReactedByMe ? '' : undefined}
      className={cn(
      ...
      >
        {visible.map((r) => (
          ...
          >
            ...
            {!isCompact && <span className="font-medium tabular-nums">{r.count}</span>}
          </ReactionChip>
        ))}
        {hasAddButton && onAdd && (
          <button
