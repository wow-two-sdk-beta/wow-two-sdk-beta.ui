...
- **WAVE 1 COMPLETE (2026-07-10):** `useOptimisticMutation` shipped (`/query` sibling hook — multi-target apply/exact-snapshot rollback, `defineEndpoint` targets, 8 tests; smart-qr delete → 5-liner with rollback) + `/feedback` subpath shipped (headless `createFeedbackBus`/`notify` · `feedbackQueryErrors()` for the `createQueryClient({onError})` seam · `<FeedbackToasts/>` adapter wrapping `Toaster`; 19 tests). Suite 1450 green. Known repo-wide finding: `eslint-plugin-boundaries` is INERT (no TS resolver — element rules never fire); fix chip filed.
- **Wave 2 started (2026-07-11):** boundaries enforcement FIXED (TS resolver + `no-unknown`; matrix was decorative until now) · forms verdict in [`forms-engine.md`](./forms-engine.md) (TanStack default + house micro-engine hedge, RHF deferred/design-checked; Standard Schema interface, zod 4 default app lib) · **`/forms-engine` COMPLETE** — contract + `house` engine + `tanstack` adapter, BOTH passing the same 34-case conformance suite (plug-and-play proven: engine pinned in one app-side re-export line); bundle isolation verified (`/forms-engine` + `/house` pull zero TanStack). Suite 1550 green.
- Remaining Wave 2: `foundation/config` typed env · storage v2. Queued: transcript-forge vendored-code deletion, product adoptions of Wave-1 modules.
