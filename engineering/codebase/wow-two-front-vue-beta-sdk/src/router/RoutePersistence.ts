 ...
 * function.
 *
 * React's null-rendering `<RoutePersistence>` becomes a hook pair. The restore is a `beforeEach`
 * REDIRECT rather than React's mount-effect `navigate(…, { replace: true })` — it lands before the
 * root route ever renders, so there is no flash of the wrong page and no history entry to replace.
 *
...
export function installRoutePersistence(router: Router, options: RoutePersistenceOptions = {}): () => void {
  ...
  // navigation and must not be hijacked. React used a ref for the same reason (plus StrictMode).
