...
export interface StepInfo {
  id: string;
  /** React typed this `ReactNode`; it is scalar here because `WizardSteps` renders it as
...
/**
 * The seam between `Wizard` and its `Steps` / `Step` / `Footer` children.
 *
 * React attached those as `Wizard.Steps` / `.Step` / `.Footer` statics over a `createContext`,
...
export const wizardContextKey: InjectionKey<WizardContextValue> = Symbol('wow-two.wizard');

/** Reads the surrounding wizard context; throws when used outside a `<Wizard>`. */
