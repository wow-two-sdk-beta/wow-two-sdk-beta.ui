...
Source: [PasswordInput.vue](PasswordInput.vue).

Public import: `import { PasswordInput } from '@wow-two-beta/ui-vue/presentation/forms';`.
...
| `hasToggle` | `boolean` | no | `true` | The visibility-toggle button's presence. Default true. |
...
| `autocomplete` | `string` | no | `undefined` | The autofill hint. Defaults to `current-password`. |
| `autoComplete` | `string` | no | `undefined` | camelCase alias of {@link PasswordInputProps.autocomplete}; the DOM spelling wins. |
...
|---|---|---|
| `update:modelValue` | `'update:modelValue': [value: string];` | Fires when the reader edits the password — the `v-model` half. |
