...
/* eslint-disable-next-line @typescript-eslint/no-empty-object-type -- kept as an
   exported name: React declared `EditableButtonProps` (an alias of
   `ButtonHTMLAttributes<HTMLButtonElement>`) and consumers import it, and both
   `EditableSubmit` and `EditableCancel` are typed by it. Every attribute falls through. */
...
/** The commit button. Rendered only while editing. */
defineOptions({ name: 'EditableSubmit', inheritAttrs: false });
