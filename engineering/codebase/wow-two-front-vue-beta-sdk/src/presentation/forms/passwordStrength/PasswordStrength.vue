...
defineOptions({ name: 'PasswordStrength', inheritAttrs: false });
...
const label = computed(() => LABELS[resolvedScore.value] ?? '');
const tone = computed(() => TONE[resolvedScore.value] ?? 'bg-destructive');
