...
import { useControlled, useId, useReducedMotion } from '../../../foundation/hooks';
 ...
 *
 * `TourStep.title` / `.body` narrow React's `ReactNode` to `string`: steps are
 * plain data passed as a prop, not slot content, and the announcement already
 * required a string.
 */
...
const emit = defineEmits<{
  ...
  /** Replaces React's `onComplete`. Fires when Done is pressed on the last step. */
  complete: [];
  /** Replaces React's `onSkip`. Fires on Skip and on Escape. */
  skip: [];
}>();
...
<template>
  <Portal v-if="mounted && step">
    ...
    <Presence v-if="tooltipCoords" :is-present="open">
      <div
        role="dialog"
        aria-modal="false"
