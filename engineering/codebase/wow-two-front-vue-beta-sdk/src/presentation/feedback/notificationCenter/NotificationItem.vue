...
export interface NotificationItemProps {
  ...
  /** The primary title. Rich content → the `title` slot. Required in React; optional here so the slot form is usable — supply one or the other. */
  title?: string;
...
/** A single notification row inside `NotificationCenter`. */
...
const emit = defineEmits<{
  /**
   * Replaces React's `onSelect`; binding it makes the row interactive
   ...
   */
  select: [];
  /** Replaces React's `onDismiss`; when bound, a dismiss button renders on hover. */
  dismiss: [];
}>();
