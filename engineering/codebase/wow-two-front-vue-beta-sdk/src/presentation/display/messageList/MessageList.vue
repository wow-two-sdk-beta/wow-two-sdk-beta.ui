<script lang="ts">
/** The imperative surface exposed on a `MessageList` template ref. */
...
/**
 * Scrolling message stream with sticky auto-scroll and a jump-to-latest
 * affordance. Messages stay in the default slot — `MessageList` owns the
 * viewport, not the rows.
 *
 * React's `header` / `footer` were structural `ReactNode` props with no string
 * form worth keeping; they are slots only.
 */
defineOptions({ name: 'MessageList', inheritAttrs: false });
...
/** React's `useImperativeHandle` surface (`MessageListHandle`), plus the root element. */
...
<template>
  <div ref="el" v-bind="rest" :class="classes">
    ...
    <button
      ...
      class="absolute bottom-3 right-3 inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background text-foreground shadow-md hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
