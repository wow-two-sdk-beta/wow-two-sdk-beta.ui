 ...
 */
defineOptions({ name: 'SwipeActions', inheritAttrs: false });

defineSlots<{
  /** The row body — React's required `children`. */
  default(): unknown;
...
/**
 * Number of action slots in a side. React counted `ReactNode` children with
 * `Children.toArray`; here the slot's vnodes are counted, flattening fragments
 * so a `v-for` sizes one slot per iteration — the snap width is
...
/** Reactive, unlike React's `useRef`: the content transition keys off it. */
const startX = ref<number | null>(null);
const offset = ref(0);
const openSide = ref<Side>(null);

/* Mutable non-render values — React's remaining `useRef`s. */
let startOffset = 0;
...
function onPointerMove(e: PointerEvent): void {
  ...
  if (Math.abs(e.clientX - startX.value) > CLICK_SUPPRESS_SLOP) {
    suppressClick = true;
  }
...
/** Vue does not append `px` to a numeric `:style` value the way React did, so every length is spelled out. */
