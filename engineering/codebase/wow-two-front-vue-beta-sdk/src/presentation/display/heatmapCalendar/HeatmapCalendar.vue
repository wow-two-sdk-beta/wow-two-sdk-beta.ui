...
const toneSteps = computed(() => TONE_CLASSES[props.tone]);
...
const weekdayOrder = computed(() =>
  Array.from({ length: 7 }, (_unused, i) => props.weekdayLabels[(i + props.weekStart) % 7]!),
);

/**
 * Per-cell render model. `attrs` carries only the bindings the cell actually
 * needs, so a non-interactive cell never gets a click listener — the Vue
 * equivalent of React's `onClick={interactive ? … : undefined}`.
 */
    ...
    cells: col.map((cell) => {
      ...
      return {
        ...
        class: cn(
          ...
          isInteractive &&
            'cursor-pointer hover:ring-1 hover:ring-ring focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
        ),
      };
    }),
  })),
