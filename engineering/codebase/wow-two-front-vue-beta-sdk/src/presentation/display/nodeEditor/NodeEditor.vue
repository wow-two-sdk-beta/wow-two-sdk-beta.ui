...
export interface NodeEditorNode {
  ...
  /**
   * The node caption. React took a `ReactNode`; a node is an array entry and
   * cannot become its own slot, so the scalar stays here and the `node` scoped
   * slot (React's `renderNode`) is the rich override.
   */
  label?: string | number;
}
...
const emit = defineEmits<{
  /** Fires with the full node list after a drag moves one of them. */
  'nodes-change': [nodes: ReadonlyArray<NodeEditorNode>];
  /** Fires with the clicked edge. */
...
/** Plain variable, not a ref — React held this in `useRef`; nothing renders off it. */
...
// scrolling. Registered once and reading `minZoom` / `maxZoom` live, which is
// what React's `[minZoom, maxZoom]` dependency array re-registered for.
...
/** Vue does not append `px` to a numeric `:style` value the way React does — every length is spelled out. */
...
const SVG_STYLE = { overflow: 'visible', width: '100%', height: '100%' } as const;
...
<template>
  ...
  >
    ...
    <div class="absolute inset-0 origin-top-left" :style="transformStyle">
      <svg class="absolute pointer-events-none" :style="SVG_STYLE">
        <g
