 ...
 *
 * React's `avatar` / `footer` were structural `ReactNode` props with no string
 * form worth keeping; they are slots only. `author` / `timestamp` keep their
 * prop form and gain same-named slots.
 */
defineOptions({ name: 'ChatBubble', inheritAttrs: false });
...
const STATUS_ICON: Record<ChatStatus, Component> = {
  sending: Clock,
  sent: Check,
...
const STATUS_ICON_CLASS: Record<ChatStatus, string> = {
  sending: 'h-3 w-3',
  sent: 'h-3 w-3',
  delivered: 'h-3 w-3',
