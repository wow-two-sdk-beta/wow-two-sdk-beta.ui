...
export interface UndoShortcutOptions extends HotkeyOptions {
  ...
  readonly undoChords?: MaybeRefOrGetter<string | readonly string[]>;

  /** Chord(s) that trigger `redo()`. Defaults to `['mod+shift+z', 'mod+y']`. */
  readonly redoChords?: MaybeRefOrGetter<string | readonly string[]>;
}
...
export function useUndoShortcuts(target: UndoRedoTarget, options?: UndoShortcutOptions): void {
  ...
  useHotkeyMap(() => {
    ...
    for (const chord of toChordList(toValue(undoChords) ?? DEFAULT_UNDO_CHORDS)) {
      map[chord] = () => {
        target.undo();
      };
    }
    for (const chord of toChordList(toValue(redoChords) ?? DEFAULT_REDO_CHORDS)) {
      map[chord] = () => {
        target.redo();
      };
    }
