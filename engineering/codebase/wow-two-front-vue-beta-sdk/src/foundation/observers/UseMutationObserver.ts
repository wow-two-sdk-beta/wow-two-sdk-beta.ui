...
//
// SUBSCRIPTION LIFECYCLE IS A PLAIN POST-FLUSH EFFECT. The React original needed a no-deps effect plus a
// signature fingerprint because a `RefObject` mutating does not re-render; in Vue a template ref is reactive, so
// `watchPostEffect` re-runs exactly when the node or the options change and never otherwise. `mutationSignature`
// stays exported from the core for a non-Vue caller, but this file has no use for it.
//
