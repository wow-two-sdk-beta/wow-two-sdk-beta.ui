...
// `facingModeConstraints` + `switchCamera` (front / back selection), and `useMediaStream` (the Vue binding
// that owns a stream's lifecycle). No components — a camera preview is a consumer of these rules, not their
// owner; Vue appears only in `UseMediaStream`.
//
...
//    own), and `useMediaStream` calls it on scope disposal — including for a stream that arrives *after*
//    disposal, when the user answered the prompt late. See `StopMediaStream.ts` and `UseMediaStream.ts`.
//
