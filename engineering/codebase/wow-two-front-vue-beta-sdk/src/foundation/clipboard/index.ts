...
// (`getPasteItems`, `usePasteHandler`), capability detection, the permission read, and the Vue binding
// `useClipboardCopy`. No HTTP, no components — a copy button is a consumer of these rules, not their owner;
// Vue appears only in the two `Use*` modules.
//
...
//
// WHY THE RESULT AND NOT REACTIVE STATE. `foundation/hooks`' `useClipboard` (which still ships, and which this
// slice deliberately does not touch) resolves its `copy` on both success and failure, recording the error in
// state instead. That makes the outcome invisible to every caller that is not a component — the reason
// `foundation/share`'s `shareOrCopy` declined to use it and open-codes `navigator.clipboard.writeText` for its
// fallback leg. Here even the composable's `copy` resolves to the full result, so the awaiting caller branches
// on the outcome while `status` drives the render.
//
