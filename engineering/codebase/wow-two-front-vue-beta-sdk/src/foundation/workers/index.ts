...
// (main thread), `exposeWorkerApi` (worker thread), `useWorker` (Vue lifetime), and `runInWorker` (a
...
// creates in `onMounted` (which does not run on a server), `exposeWorkerApi` finds no worker scope on the
