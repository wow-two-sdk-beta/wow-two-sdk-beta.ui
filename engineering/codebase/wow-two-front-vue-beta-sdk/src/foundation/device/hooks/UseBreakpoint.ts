...
//
// Every media query in this library must go through `foundation/device`' `useMediaQuery`, so there is exactly one
...
//
// The arity is fixed because a composable's cleanup is registered on the surrounding effect scope, so
// subscriptions created in a loop are only released when the WHOLE scope dies. A scale that shrank mid-session
// would leak a live `MediaQueryList` per dropped entry. A FIXED number of `useMediaQuery` calls —
// `MaxBreakpoints` of them, always — has no such leak, and the slots a smaller scale leaves over are parked on
// `'not all'`, a valid media query that can never match.
//
// Each slot takes a GETTER rather than a fixed string, so a changed scale re-points the existing subscriptions
// instead of needing a remount.
//
// The slot count below MUST equal `MaxBreakpoints`. Queries arrive widest-first, so a scale with more entries
...
/** A valid media query that never matches — parks the slots a scale smaller than `MaxBreakpoints` leaves unused. */
const NeverMatches = 'not all';

/** Reads slot `index`'s query string, falling back to the never-matching parking query. */
function queryAt(queries: ReadonlyArray<BreakpointQuery>, index: number): string {
  return queries.at(index)?.query ?? NeverMatches;
}
 ...
 * `TailwindBreakpoints` to opt into Tailwind's defaults, or your own map to match your tokens.
 *
 ...
 * items to prefetch.
 *
 * @param scale Breakpoint key to minimum viewport width in CSS pixels. Entries beyond `MaxBreakpoints`, counted
...
export function useBreakpoint<TScale extends BreakpointScale>(
  ...
  return computed(() => {
    // Exactly `MaxBreakpoints` entries — the invariant the header states. Positionally aligned with `queries`.
    const matches: ReadonlyArray<boolean> = [
      match0.value,
      match1.value,
      match2.value,
      match3.value,
