 ...
 *
 * The React version was a hand-rolled listener store read through `useSyncExternalStore`; here
 * it is one immutable object in a `shallowRef`, so a read of `state.value` is the subscription.
 * TanStack's own store stays a `@tanstack/store` `Store` — the adapter bridges it separately.
 */
  ...
  return {
    ...
    reset: (baseline) => {
      commit({
        ...
        touched: new Set<string>(),
        submitCount: 0,
        isSubmitting: store.value.isSubmitting, // an in-flight submit still owns its flag (its finally clears it)
        submitError: null,
        isSubmitSuccessful: null,
      });
    },
