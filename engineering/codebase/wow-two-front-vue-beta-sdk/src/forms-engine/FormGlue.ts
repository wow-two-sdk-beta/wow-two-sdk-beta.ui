 ...
 *
 * `createSubscribeComponent` has no Vue counterpart — the render-prop subscription it built is
 * what `form.state` / `form.values` replace (see `AppForm.ts`). `Field` stays a component
 * because `provide()` requires one. In its place this module carries the three live-getter
 * factories BOTH adapters build their reactive surface from — the work React's per-adapter
 * `useEngineField` / `useEngineSlice` hooks did, now engine-free because there is no hook
 * involved: a getter reads the engine's `shallowRef` and whoever reads the getter is subscribed.
 *
 ...
 * re-renders on its own edits and sits still through everyone else's. It is the bail-out React got
 * from `useSyncExternalStore`'s selector cache, in one line.
 *
