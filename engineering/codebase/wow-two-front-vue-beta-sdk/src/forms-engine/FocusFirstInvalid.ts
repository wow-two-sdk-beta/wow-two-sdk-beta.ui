 ...
 *
 * Call it after `handleSubmit` settles; capture the form element BEFORE awaiting — the DOM
 * nulls `event.currentTarget` once dispatch ends, so reading it after an `await` yields `null`:
 *
 * ```vue
 * <form @submit="(event) => {
 *   const root = event.currentTarget as ParentNode;
